...
CTCP::CTCP(Optical_Detection_UI* object)
{
    m_object = object;

    bool Ret = true;
    m_ClientSocket = new QTcpSocket(m_object);
    if(m_ClientSocket)
    {
        Ret = m_object->connect(m_ClientSocket, SIGNAL(readyRead()), m_object, SLOT(RecvDataFromTCP()));
        Ret = m_object->connect(m_ClientSocket, SIGNAL(connected()), m_object, SLOT(ConnectedToTCP()));
        Ret = m_object->connect(m_ClientSocket, SIGNAL(disconnected()), m_object, SLOT(disConnectedToTCP()));
        Ret = m_object->connect(m_ClientSocket, SIGNAL(error(QAbstractSocket::SocketError)), m_object, SLOT(tcpError(QAbstractSocket::SocketError)));
        if(!Ret) qDebug() << "fail to connect tcp signal";
    }
...
/*
    Methods
*/
int CTCP::ConnectToTCP(QString ip, quint16 port)
{
    m_ClientSocket->connectToHost(ip, port);
    return 0;
}

int CTCP::CloseClientSocket()
{
    m_ClientSocket ->close();
    return 0;
}

int CTCP::RecvDataFromTCP()
{
    const QByteArray buffer = m_ClientSocket->readAll();
...
int CTCP::ConnectedToTCP(QString wfname)
{
    qDebug() << "ConnectedToTCP";
    qDebug() << "WIFI Name:" << wfname.toLocal8Bit().data();
    qDebug() << m_ClientSocket->state();
    return 0;
}

int CTCP::disConnectedToTCP()
{
    qDebug() << "disConnectedToServer";
    return 0;
}

int CTCP::SendCmdToTCP(quint16 Cmd)
{
    if(m_ClientSocket == nullptr) return SOCKET_ERROR;
    ...
    case COMM_CMD_SAVE_VERSION:
    {
        // ',' + 8자리 문자열 = 9byte
        char strBuf[10] = { 0, };
        CmdBuf = (char*)malloc(sizeof(Cmd) + sizeof(strBuf) + sizeof(strEOC));
        if (CmdBuf == NULL) return Ret;
        memcpy(CmdBuf, &Cmd, sizeof(Cmd));
        sprintf(strBuf, ",%s", m_Send_stVERSIONINFO.ModuleID);
        memcpy(CmdBuf + sizeof(Cmd), strBuf, sizeof(strBuf));
        memcpy(CmdBuf + sizeof(Cmd) + sizeof(strBuf), strEOC, sizeof(strEOC));
        Ret = m_ClientSocket->write(reinterpret_cast<char*>(CmdBuf), sizeof(Cmd) + sizeof(strBuf) + sizeof(strEOC));
    }
        break;

    case COMM_CMD_LOAD_VERSION:
    {
        CmdBuf = (char*)malloc(sizeof(Cmd) + sizeof(strEOC));
        if (CmdBuf == NULL) return Ret;
        memcpy(CmdBuf, &Cmd, sizeof(Cmd));
        memcpy(CmdBuf + sizeof(Cmd), strEOC, sizeof(strEOC));
        Ret = m_ClientSocket->write(reinterpret_cast<char*>(CmdBuf), sizeof(Cmd) + sizeof(strEOC));
    }
        break;
...
int CTCP::tcpError(QAbstractSocket::SocketError error)
{
    qDebug() << "Socket Error : " + QString(error);
    return error;
}
